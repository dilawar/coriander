		       ...
		       int sy, int type, int bits)
{
	...
	for (i = 0; i < sy; i += 2) {
		...
		tmp =
		    ((src[i * sx + j] + src[(i + 1) * sx + (j + 1)]) >> 1);
		CLIP16(tmp, outG[(((i * sx) >> 2) + (j >> 1)) * 3], bits);
		tmp = src[i * sx + j + 1];
		CLIP16(tmp, outR[(((i * sx) >> 2) + (j >> 1)) * 3], bits);
		tmp = src[(i + 1) * sx + j];
		CLIP16(tmp, outB[(((i * sx) >> 2) + (j >> 1)) * 3], bits);
	    }
	}
	break;
    case COLOR_FILTER_FORMAT7_BGGR:	//---------------------------------------------------------
