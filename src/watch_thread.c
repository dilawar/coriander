...
WatchThread(void *arg)
{
  ...
  while (1) {
	...
	if ((camera->camera_info.mode >= DC1394_MODE_FORMAT7_MIN) &&
	  ...
	  if ((info->size[0]==camera->format7_info.mode[camera->camera_info.mode-DC1394_MODE_FORMAT7_MIN].max_size_x)&&
	      (info->size[1]==camera->format7_info.mode[camera->camera_info.mode-DC1394_MODE_FORMAT7_MIN].max_size_y))
	    ...
	    SetFormat7Crop(info->size[0],info->size[1],
			   info->pos[0]+camera->format7_info.mode[camera->camera_info.mode-DC1394_MODE_FORMAT7_MIN].pos_x,
			   info->pos[1]+camera->format7_info.mode[camera->camera_info.mode-DC1394_MODE_FORMAT7_MIN].pos_y,
